...
// MARK: NÃO SE ESQUEÇA DO SIGNIFICADO DE CADA UM EMMMM
...
// Codable
// Codable é, na verdade, apenas uma combinação dos protocolos Decodable e Encodable. Isso significa que, se um tipo é Codable, ele pode ser usado para decodificar dados do formato JSON, bem como codificar objetos em dados JSON.
...
if let jsonData = jsonString.data(using: .utf8) {
    do {  //               tentar                        objeto swift       JSON
        let person: Person = try JSONDecoder().decode(Person.self, from: jsonData)
        print("Decodificado com sucesso:")
        print("Nome: \(person.name ?? "Sem nome"), Idade: \(person.age ?? 0), Sobrenome: \(person.lastName ?? "")")
    } catch {
...
let person: Person = Person(name: "Caio", age: 55, lastName: "Fabrini")
...
do {
    ...
    if let jsonString = String(data: jsonData, encoding: .utf8) {
        print("JSON codificado com sucesso:")
        print(jsonString)
    }
} catch {
